...
@Component({
  selector: 'app-doktor-pacient-zaznamy-change',
  templateUrl: './doktor-pacient-zaznamy-change.component.html',
  styleUrls: ['./doktor-pacient-zaznamy-change.component.css']
})
export class DoktorPacientZaznamyChangeComponent implements OnInit {
  osobneCislo:string = "";
  rodneCislo:string = "";
  ...
  delete(vystrenie:VysetrenieZaznam) {
    this.doctorService.deleteVysetrenie(vystrenie.kod,vystrenie.idzaznam).subscribe(
      (result:VysetrenieZaznam[]) => {
        const id = Number(this.route.snapshot.paramMap.get('idzaznam'));
        this.pacientService.getVysetreniaVZazname(id).subscribe(
          (result:VysetrenieZaznam[]) =>
